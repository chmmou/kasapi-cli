## kasapi-cli config init

Interactively create or replace a profile in the config file

```
kasapi-cli config init [flags]
```
...
```
  ...
  -h, --help          help for init
      --name string   profile name to write (the persistent --profile flag selects which profile is *used* at runtime) (default "main")
```
